...
def ex6(evaluate_time, learning_mode, incremental):
    action_size = 4
    state_size = 100
    ...
    while trials < 30:
        ...
        results.append(trainer(q_matrix, alpha, discount, 20000, learning_mode, evaluate_time, incremental, greed))
        global greed_weight
        global no_greed_weight
        greed_weight = 0.3
        no_greed_weight = 0.7
        if trials == 29:
            heat_map(q_matrix, learning_mode, alpha, discount, greed, incremental)
        trials += 1
    ...
    plt.title('Alfa=' + str(alpha) + ' Discount=' + str(discount) + ('' if incremental else (
            ' Greed=' + str(greed * 100) + '%')))
    if evaluate_time:
...
def trainer(q_matrix, alpha, discount, times, learning_mode, evaluate_time, incremental, greed):
...
def heat_map(q_matrix, learning_mode, alpha, discount, greed, incremental):
...
if __name__ == '__main__':
    ex6(evaluate_time=True, learning_mode=True, incremental=True)
